...
contract ExerciseTracker is AccessControl {
    ...
    function logExercise(
        ...
        exerciseRecords[patient].push(
            ExerciseRecord(tokenId, block.timestamp, duration, isCorrectForm)
        );
    }
    ...
    function getExerciseData(
        address patient,
        uint256 tokenId
    ) external view onlyRole(DOCTOR_ROLE) returns (ExerciseRecord[] memory) {
        ExerciseRecord[] memory records = new ExerciseRecord[](
            exerciseRecords[patient].length
        );
        uint256 count = 0;
