...
async function main() {
    ...
    const GovernmentOracle = await ethers.deployContract("GovernmentOracle", [DIDRegistry.target]);
    await GovernmentOracle.waitForDeployment();
    console.log("GovernmentOracle Contract Deployed at:", GovernmentOracle.target);
    ...
    const deployments = {
        ...
        GovernmentOracle: {
            address: GovernmentOracle.target,
            deploymentHash: GovernmentOracle.deploymentTransaction()?.hash
        },
        // PhysioToken: {
    ...
    try {
        const fs = require('fs');
        fs.writeFileSync(
            'scripts/lisk-deployments-2.json',
